...
logd("Have writen the PID file at /var/run/wifidbd.pid." $log_interval, ,  $log_level);
verbose("PID Writen", 1); 
// since we now have a valid PID to make the Daemon run, lets get some configurations variables that you have set.
include('config.inc');
...
include($wifidb_install."/lib/config.inc.php");
logd("Have included the WiFiDB installed Config.inc.php file at ".$wifidb_install."/lib/ .", $log_interval, ,  $log_level);
verbose("Have included the WiFiDB installed Config.inc.php file at ".$wifidb_install."/lib/ .", 1, ,  $log_level); 

include('functions.php'); //need to include the functions file so that the daemon can do things...
logd("Have included the WiFiDB Tools Functions file for the 'Daemon'.", $log_interval, ,  $log_level);
verbose("Have included the WiFiDB Tools Functions file for the 'Daemon'.", 1 , ,  $log_level); 
...
//Main loop
while(fopen($pid_file, "r")) //while my pid file is still in the /var/run/ folder i will still run, this is for the init.d script or crash override
{
	if(mysql_query("SELECT * FROM `files_tmp`", $conn))//check to see if i can successfully look at the file_tmp folder
	{
		while ($files_array = mysql_fetch_array($result))//got through every row in the table that is returned
		{
			if($files_array['file'] != "")//make sure there is atleast somthing in the file field
			{
				...
				$check = $database->check($files_array['file']);//check to see if this file has aleady been imported into the DB
				if($check == 1)
				{
					...
					$tmp = import_vs1($source, $files_array['user'], $files_array['notes'], $files_array['title'], $verbose);
	...
	logd("Finished Import of all files in table, going to sleep for 30 minuets.", $log_interval, ,  $log_level);
