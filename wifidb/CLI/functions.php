...
function logd($message = '', $log_interval = 0, $details = '',  $log_level = 0)
{
	...
	{
		$cidir = getcwd();
		$filename = $cidir.'\log\wifidbd_log.log';
		if(!is_file($filename))
		{
			fopen($filename, "w");
		}
		$fileappend = fopen($filename, "a");
		if($log_level == 2 && $details == ''){$log_level = 1;}
		if($log_level == 2)
		{
			$message = $message."\n==Details==\n".$detail."\n===========\n";
		}
	...
	}elseif($log_interval==1)
	{
		$date = date("y-m-d");
		$cidir = getcwd();
		$filename = $cidir.'\log\wifidbd_'.$date.'_log.log';
		if(!is_file($filename))
		{
			fopen($filename, "w");
		}
...
{
	include($wifidb_install."/lib/database.inc.php");
	...
	if ($source == NULL)
	{
		logd("There was an error sending the file name to the function", $log_interval, ,  $log_level);
	...
	if($count <= 8) 
	{
		logd("You cannot upload an empty VS1 file, at least scan for a few seconds to import some data.", $log_interval, ,  $log_level);
	...
	foreach($return as $ret)
	{
		...
		}elseif($ret_len == 13)
		{
				...
				if(!isset($SETFLAGTEST))
				{
					...
					if($count == 0) 
					{
						logd("This File does not have any APs to import, just a bunch of GPS cords.", $log_interval, ,  $log_level);
				...
				if(strcmp($table,$table_ptb)===0)
				{
					...
					logd($FILENUM." / ".$count."   ( ".$APid." )   ||   ".$table." - is being updated ", $log_interval, ,  $log_level);
				...
				#		echo ".";
						...
						if($return_gps === 0)
						{
							...
							{
								verbose("Successful import of GPS data \r\n $gps_id - $lat - $long - $sats - $hdp - $alt - $geo - $kmh - $mph - $track - $date - $time", $verbose);
							}else
							{
								logd("There was an Error inserting the GPS information" , $log_interval, ,  $log_level);
					...
					logd($user_aps[$user_n], $log_interval, ,  $log_level);
				...
				{
					// NEW AP
					logd($FILENUM." / ".$count."   ( ".$size." )   ||   ".$table." - is Being Imported", $log_interval, ,  $log_level);
	...
	#				echo "(3)Insert into [".$db_st."].{".$table."}\n		 => Add Signal History to Table\n";
		...
		#					echo 'Updated ['.$db.'].{'.$wtable."} with new Size \n		=> ".$size."\n";
							logd("Updated Settings table with new size", $log_interval, ,  $log_level);
		...
		{
			logd("Text files are no longer supported, please save your list as a VS1 file or use the Extra->Wifidb menu option in Vistumbler", $log_interval, ,  $log_level);
		...
		{
			logd("There is something wrong with the file you uploaded, check and make sure it is a valid VS1 file and try again", $log_interval, ,  $log_level);
