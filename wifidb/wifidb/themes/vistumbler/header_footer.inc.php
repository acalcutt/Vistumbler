...
function pageheader($title, $output="detailed", $install=0)
{
	...
	if($output == "detailed")
	{
		echo '<html><head><title>Wireless DataBase '.$GLOBALS["ver"]["wifidb"].' --> '.$title.'</title>'.$head."\r\n";
		...
		if($title == "Search Page")
                {
                    ?><body onload="init()" style="background-color: #145285"><?php
                }else{
                    ?><body style="background-color: #145285"><?php
		}
                ?>
