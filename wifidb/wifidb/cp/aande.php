...
if($usersearch != '')
{
	...
	{
		$email_search = 1;
		$emailsearch = addslashes($_POST['email']);
	}
}else
{
	$user_search = 0;
...
switch($func)
{
	case "update":
		...
		{
			$sql0 = "SELECT * FROM `$db`.`$user_logins_table` WHERE `disabled` != '1' AND `email` LIKE '$emailsearch' AND `h_email` != '1' AND `username` NOT LIKE 'admin%'";
		}else
		{
		...
		<td style="background-color: #A9C6FA;width: 80%;vertical-align: top;" align="center">
				...
				<form method="post" action="./?func=update_<?php if($type == "foes"){ echo "foes"; }else{ echo "friends"; }?>">
					...
					<table width="100%" align="center">
						...
						<tr>
							<th class="style4">Username</th>
							<th class="style4">Level</th>
							<th class="style4">APs / GCs</th>
							<th class="style4">Website</th>
							<th class="style4" width="20%">Joined</th>
							<th class="style4" width="20%">Last Login</th>
						</tr>
						<?php
						################
						if($rows > 0)
						{
							while($newArray = mysql_fetch_array($result))
							{
								...
								if($newArray['admins'])
								{$userprivname = "Administrator";}
								elseif($newArray['mods'])
								{$userprivname = "Moderator";}
								elseif($newArray['devs'])
								{$userprivname = "Developer";}
								elseif($newArray['users'])
								{$userprivname = "User";}
						#		echo $userprivname;
						...
						#		echo $sql1."<BR>";
								...
								while($aps_array = mysql_fetch_array($result1))
								{
									...
									foreach($pnts_exp as $pnts)
									{
										$pnt_exp = explode(",", $pnts);
										if($pnt_exp[0] == 1){continue;}
										$total_aps += 1;
									}
								}
								if($total_aps > 0)
								{
									$username_ = '<a href="'.$hosturl.$root.'/opt/userstats.php?func=alluserlists&user='.$user_name.'" class="links">'.$user_name.'</a>';
								}else
								{
									$username_ = $user_name;
								}
								...
								if($website == '' or $website == "No Website")
								{
								...
								}else
								{
									...
									$website_name = '<A href="'.$website.'" target="_blank">'.$website_exp[$web_count-1].'.'.$website_exp[$web_count].'</a>';
								}
								?>
								<tr class="<?php echo $style_class;?>">
									<td><input type="checkbox" name="user<?php echo $user_number;?>" value="<?php echo $user_name;?>" />&nbsp;&nbsp;<?php echo $username_; ?></td>
									<td align="center"><?php echo $userprivname;?></td>
									<td align="center"><?php echo $total_aps." / ".$shared_gcs;?></td>
									<td align="center"><?php echo $website_name;?></td>
