...
verbose("Log Level is: ".$GLOBALS['log_level']." (".$de.")", $verbose);
if($log_level != 0)
{
	...
	verbose("Log Interval is: ".$GLOBALS['log_interval']." (".$de.")", $verbose);
}
	...
	if($result)//check to see if i can successfully look at the file_tmp folder
	{
		while ($files_array = mysql_fetch_array($result))//got through every row in the table that is returned
		{
			...
			}else
			{
				...
				verbose("File tmp table is empty, go and import something.\n", $verbose);
			}
